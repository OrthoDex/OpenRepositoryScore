...
const storeModel = (model, path) => {
  try {
    const modelSaver = tf.train.Saver();
    return modelSaver.save(model, path);
  } catch (error) {
    console.error(error);
...
const loadToxicityModel = async () => {
  try {
    const model = await tf.loadLayersModel("indexeddb://tf-toxicity-model");
    console.log("Loading cached model");
    return model;
  } catch (error) {
    console.log("Could not load cached model.");
    console.error(error);
  }

  const model = await toxicity.load(threshold);
  try {
    storeModel(model, "indexeddb://tf-toxicity-model");
    console.log("saved toxicity model to indexeddb");
  } catch (error) {
    console.log("couldn't save model");
    console.error(error);
  }
